...
def get_best_results(data_df, metric, metric_name, max_o_min='min'):
    for grid_size in list_grid_size:
        if max_o_min == 'max':
            best = max(data_df[(data_df['grid_size'] == grid_size)][metric])
        elif max_o_min == 'min':
            best = min(data_df[(data_df['grid_size'] == grid_size)][metric])
        else:
            best = 'error'
        print('Grid size: {} - Metric: {} - Best value: {}'.format(grid_size,metric_name, best))
...
max_o_min = 'max' if i > 6 else 'min'
get_best_results(data, used_metrics_list[i], used_metric_names_list[i], max_o_min)
